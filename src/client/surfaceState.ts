...
export interface HeroPillInputs {
  ...
  /** The failure of the latest reading, or null. */
 ...
 * says, because it is the reason the reading failed. A terminal reading failure outranks a missing
 * view, because "not a repository" is already a complete answer. Only then is a missing reading
 * treated as still loading — so a project that is not a repository never flashes "loading" after it
 * has been told, and a repository never flashes "no git" before it has been read.
...
export function heroPillState(inputs: HeroPillInputs): HeroPillState {
  const { hasProject, view, overview, failure } = inputs
